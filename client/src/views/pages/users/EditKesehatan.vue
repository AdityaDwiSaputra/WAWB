    ...
    <v-container>
        ...
        <v-row no-gutters>
            <v-col sm="10" class="mx-auto">
                <v-card class="pa-5">
                    <v-card-title>Edit Tenaga Kesehatan</v-card-title>
                    <v-divider></v-divider>
                    <form class="pa-5"  enctype="multipart/form-data">
                        ...
                        <v-text-field v-model="user.kesehatan.jabatan" label="Jabatan" variant="outlined" prepend-icon="mdi-face-agent" :rules="rules"></v-text-field>
                        <v-file-input @change="selectedFile" variant="outlined" :rules="rules" show-size counter multiple label="Select Foto"></v-file-input>
                        <v-img width="150" :src="`/${user.kesehatan.foto}`"></v-img>
                        <v-btn @click="submitForm" class="mt-3" color="success">Update Tenaga Kesehatan</v-btn>
                    </form>
                </v-card>
            </v-col>
        </v-row>
...
export default {
    ...
    methods: {
        ...
        async submitForm(){
            ...
            formData.append('jabatan', this.user.kesehatan.jabatan);
            formData.append('old_image', this.user.kesehatan.foto)
            await axios
